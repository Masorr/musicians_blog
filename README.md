...
- This was done since the error was related about a "null" value passed to this database table. I.e. the database is waiting for a value which is never being sent. It could be seen in the table that there is an author_id, post_id and user_id, of which the user_id appeared both suspicious and redundant. The command was run to adjust the table data without losing comment data or having to undo migrations. It seems that there must've been some corruption in the database table.
